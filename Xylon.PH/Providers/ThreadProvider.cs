...
namespace ProcessHacker
{
    ...
    public class ThreadProvider : Provider<int, ThreadItem>
    {
        ...
        public class ResolveSybolData : EventArgs
        {
            public int TID;
            public ulong StartAddress;
            public ResolveMessage ResolveInfo;
        }

        public class ResolveMessage : Message
        {
            public int Tid;
            ...
            : base()
        {
            ...
            catch
            {
                try
                {
                    ...
                    {
                        _processAccess = OSVersion.MinProcessQueryInfoAccess;
                        _processHandle = new ProcessHandle(_pid, _processAccess);
                    }
                    else
                    {
                        _processAccess = ProcessAccess.QueryInformation;
                        _processHandle = new ProcessHandle(_pid, _processAccess);
                    }
                }
            ...
            _symbolsWorkQueue.QueueWorkItemTag(new Action(() =>
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            using (var phandle =
                                new ProcessHandle(_pid, OSVersion.MinProcessQueryInfoAccess | ProcessAccess.VmRead))
                            {
        ...
        public delegate void MyEventHandler(object sender, ResolveSybolData e);

        public event MyEventHandler ResolveSybol;
        ...
        private void ResolveThreadStartAddress(int tid, ulong startAddress)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    ResolveSybolData TempResolve = new ResolveSybolData();
                    TempResolve.TID = tid;
                    TempResolve.StartAddress = startAddress;
                    TempResolve.ResolveInfo = result;

                    this.ResolveSybol.Invoke(null, TempResolve);
        ...
        protected override void Update()
        {
            ...
            foreach (int tid in threads.Keys)
            {
                ...
                if (!Dictionary.ContainsKey(tid))
                {
                    ...
                    try
                    {
                        item.ThreadQueryLimitedHandle = new ThreadHandle(tid, OSVersion.MinThreadQueryInfoAccess);
